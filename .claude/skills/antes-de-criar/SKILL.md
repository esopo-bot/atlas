---
name: antes-de-criar
description: Use quando a pergunta for "já existe CÓDIGO assim?" — antes de criar serviço, helper, contrato, componente ou funcionalidade nova, ao pedirem para ver se algo assim já está nos repositórios do workspace, ou ao começar aplicação do zero. Procure e cite o que achou antes de escrever a primeira linha. Vale para peça de código e aplicação, não para texto — escrever ou atualizar documentação é da documentar-processo, e abrir issue sobre a tarefa é da trabalho-por-issue.
metadata:
  pedidos-de-exemplo:
