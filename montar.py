...
# Arquivos de configuração: iguais em qualquer repositório.
PERMISSOES = (
    '  "permissions": {\n'
    '    "allow": [],\n'
    '    "deny": [\n'
...
# A lista do veto de branch protegida. Ela é da CASA, não da camada: nasce uma
# vez e a atualização nunca a reescreve. Por isso vive aqui e não em FONTES —
# o código do gancho recebe conserto, a lista não. E é criada tanto na montagem
# (via ARQUIVOS) quanto pelo `garantir_lista_protegida`, porque `--atualizar`
# não escreve ARQUIVOS: sem a segunda porta, quem já tinha a camada receberia o
# gancho e ficaria sem a lista para editar.
LISTA_PROTEGIDA = (
    ...
    "# aqui e acrescente os nomes da sua casa — este arquivo é seu, e a\n"
...
ARQUIVOS = {
    # As regras nascem no modelo de propósito: skill dispara em só metade dos
    # pedidos (medido); linha no AGENTS.md vale em toda sessão, sempre.
    "AGENTS.md": (
        ...
        "\n"
        "Procure o que já existe e cite o que achou (skill antes-de-criar). O mapa\n"
        "dos projetos — quando gerado pela skill wiki-de-projetos — fica em\n"
        ...
        "que vale só para esta casa se escreve lá; lição genérica muda de endereço\n"
        "para a camada.\n"
        "\n"
        "## Ao dar um trabalho por pronto\n"
        "\n"
        "Faça a análise de promoção (skill analise-de-promocao): o que nasceu\n"
        "genérico é proposto para a camada; o que é da casa vira nota em\n"
        ...
        "e de tocar em configuração de esteira. O que segue é o essencial dela.\n"
        "\n"
        ...
        "- Não invente passo onde já existe receita: procedimento que esta casa\n"
        "  já tem (subir, publicar, liberar acesso) se procura na documentação\n"
        "  dela e se cita. Não achou? Peça o endereço — esteira improvisada em\n"
        "  cima de esteira parece pronta e quebra longe de onde nasce.\n"
        ...
        "- Nunca abra arquivo de credencial (.credenciais/, .env*, appsettings*).\n"
        "- Branch de longa duração (integração, homologação, produção) e\n"
        "  configuração de esteira NÃO se tocam: não apague, não renomeie, não\n"
        ...
        "- Destrutivo é do dono. Push segue o que ESTE repositório autorizou por\n"
        "  escrito — sem registro, não empurre; e o que aciona esteira,\n"
        "  implantação ou aviso a alguém é sempre dele. Sincronizar não é entregar.\n"
        "- Só chame de pronto o que um instrumento provou (build, teste, listagem).\n"
        "- Conteúdo e comunicação em pt-BR.\n"
    ),
    # A linha do import fica sozinha e sem ponto final: um ponto colado entra
    # no caminho do arquivo e o import falha em silêncio.
    "CLAUDE.md": (
        "# CLAUDE.md\n"
        "\n"
    ...
    # Casa das skills: caminho lido por Codex, Devin, Cursor, Copilot, Gemini,
    # opencode e Zed. O Claude Code é o único que não lê daqui — por isso o
    # script espelha cada skill para .claude/skills/ (veja espelhar_skills).
    ".agents/skills/.gitkeep": "",
    ".claude/settings.json": "{\n" + PERMISSOES + "}\n",
    # skills/ é cópia gerada, mas entra no git: sessão na nuvem só enxerga o
    # que está commitado, e sem a cópia ela roda sem skill nenhuma. O
    # verificador acusa quando a cópia diverge da fonte.
    ".claude/.gitignore": "settings.local.json\n",
    ".claude/commands/.gitkeep": "",
    ".claude/skills/.gitkeep": "",
    ".claude/agents/.gitkeep": "",
    # A ponte para a .claude fica desligada: o Devin lê a fonte .agents/skills
    # sozinho, e pela ponte cada skill chegava em dobro — e permissão nenhuma
    # vinha junto. As negações valem aqui, por conta própria.
    ".devin/config.json": (
        "{\n"
        '  "read_config_from": {\n'
        '    "claude": false\n'
    ...
    ".claude/branches-protegidas.txt": LISTA_PROTEGIDA,
    # O endereço do descartável. Sem um lugar óbvio, o rascunho gerado nasce
    # na raiz com nome improvisado e fica lá para sempre.
    "tmp/LEIAME.md": (
        "# tmp\n"
        "\n"
...
# Esqueleto do workspace: só nasce com --esqueleto, porque pertence à raiz que
# abriga os repositórios — dentro de um projeto, uma pasta projetos/ não faz
# sentido. Os repositórios de código já têm git próprio; segredo não entra em
# git nenhum.
ESQUELETO = {
    "projetos/LEIAME.md": (
        "# projetos\n"
    ...
    ".credenciais/LEIAME.txt": (
        ...
        "\n"
        "Tokens de MCP: escreva NOME=valor no arquivo mcp.env e rode\n"
        "python .credenciais/publicar-mcp-env.py para publicar como variavel\n"
        "de ambiente. No .mcp.json fica so ${NOME} - segredo nunca no arquivo.\n"
    ),
    # O publicador vive junto dos tokens: lê o mcp.env ao lado dele e publica
    # como variável do usuário, sem nunca exibir um valor.
    ".credenciais/publicar-mcp-env.py": (
        '"""Publica os tokens de mcp.env como variáveis de ambiente do usuário.\n'
        "\n"
        "Rode depois de editar o mcp.env (token trocado, servidor novo):\n"
        "\n"
        "    python .credenciais/publicar-mcp-env.py\n"
        "\n"
        'Nenhum valor é exibido — só os nomes publicados.\n'
        '"""\n'
        "\n"
        "import subprocess\n"
        "import sys\n"
        ...
        '    sys.exit("mcp.env não existe ao lado deste script.")\n'
        "\n"
        "publicadas = []\n"
        ...
        '    r = subprocess.run(["setx", nome.strip(), valor],\n'
        "                       capture_output=True, text=True)\n"
        "    if r.returncode != 0:\n"
        '        sys.exit(f"FALHA ao publicar {nome}: {r.stderr.strip()[:100]}")\n'
        "    publicadas.append(nome.strip())\n"
        "\n"
        'print("Publicadas:", ", ".join(publicadas))\n'
        'print("Sessão aberta não enxerga variável nova: feche e abra o terminal ou o VS Code.")\n'
    ),
    "recursos/LEIAME.md": (
        "# recursos\n"
        "\n"
...
# e as exceções seguram a estrutura no git — pasta ignorada inteira não chega
# no clone, e a estrutura do workspace deve se replicar em toda máquina.
# Segredo continua fora: só o LEIAME e o publicador viajam da .credenciais.
IGNORAR = (
...
# Quais arquivos do repositório da camada viram páginas embutidas. As skills
# entram inteiras: sem isso, o script montaria o guia num outro repositório e
# deixaria as skills para trás, em silêncio. Os ganchos e o site viajam pelo
# mesmo caminho, para receberem conserto junto com a camada.
#
# Gancho entra NOMEADO, nunca por glob: `.claude/hooks/*.py` também varre o
# gancho local de quem trabalha aqui — que fica fora do git, lê arquivo que só
# existe nesta máquina e não faz sentido nenhum no repositório de outra pessoa.
# Glob embutiria esse arquivo dentro do script e o instalaria em toda casa.
FONTES = ("fluxos/*.md", "conhecimento/*.md", ".agents/skills/**/*",
          ".claude/hooks/injetar-qualidade.py",
          ".claude/hooks/vetar-branch-protegida.py",
          ".claude/hooks/conferir-mcp.py", "site/docusaurus.config.js",
          "site/sidebars.js", "site/package.json", "site/src/css/custom.css",
          ".markdownlint.jsonc")

# A parte OPCIONAL da camada. Ela mora fora do FONTES de propósito: o que o
# FONTES varre entra em PAGINAS, e PAGINAS chega em todo repositório que rodar
# este script. Módulo é o contrário — só chega onde alguém o pediu pelo nome.
# A garantia é estrutural: o script não olha para cá ao montar as páginas.
# Fosse uma exclusão dentro do FONTES, bastaria alguém esquecer de cadastrar
# um arquivo para ele viajar em silêncio, e o erro só apareceria na máquina
# dos outros.
#
# A fonte espelha a árvore de destino: modulos/<nome>/<o caminho de destino>.
# Não existe manifesto — o caminho do arquivo É a declaração de onde ele vai
# parar, e assim não há duas listas para dessincronizar.
PASTA_MODULOS = "modulos"

# A única exceção à regra acima: o LEIAME na raiz de um módulo diz o que aquele
# módulo é, para quem cuida da camada. Ele NÃO é instalado — instalado, iria
# parar na raiz do repositório de destino e sobrescreveria arquivo alheio.
# LEIAME de subpasta (conhecimento/<sub>/LEIAME.md) é conteúdo e viaja normal.
CARTAO_MODULO = "LEIAME.md"

# Gerados do site: se recriam com npm, não entram em git nenhum.
IGNORAR_SITE = ("node_modules/", "site/build/", "site/.docusaurus/")
MOTIVO_SITE = "# Gerados do site do guia — se recriam entrando em site/ e rodando npm install e npm run build"

# Descartáveis: o conteúdo do tmp/ e o cache que as ferramentas deixam para
# trás. A pasta tmp/ sobrevive pelo LEIAME — é o endereço do rascunho.
...
# Os comandos de gancho carregam ${CLAUDE_PROJECT_DIR} de propósito: gancho
# declarado com caminho relativo roda a partir do cwd da sessão — e o cwd
# anda quando um comando faz `cd`. Medido: com o shell parado numa subpasta,
# o gancho relativo não é encontrado e TODA chamada de ferramenta trava.
COMANDO_GANCHO = 'python "${CLAUDE_PROJECT_DIR}/.claude/hooks/injetar-qualidade.py"'
COMANDO_VETO = 'python "${CLAUDE_PROJECT_DIR}/.claude/hooks/vetar-branch-protegida.py"'
COMANDO_CONFERIR_MCP = 'python "${CLAUDE_PROJECT_DIR}/.claude/hooks/conferir-mcp.py"'

def comandos_de(bloco: dict) -> list:
    """Os comandos declarados num bloco de gancho do settings.json."""
    ganchos = bloco.get("hooks", []) if isinstance(bloco, dict) else []
    return [g.get("command", "") for g in ganchos if isinstance(g, dict)]


def tem_comando(lista: list, comando: str) -> bool:
    return any(comando in comandos_de(b) for b in lista)


def tirar_outras_formas(lista: list, arquivo: str, comando_novo: str) -> bool:
    """Remove declarações do mesmo gancho que não sejam a forma canônica.

    Pega a relativa que a camada gerava antes — quebra quando o cwd anda — e
    a absoluta que alguém fixou à mão numa máquina: caminho absoluto é
    resíduo de máquina num arquivo que entra no git, e a duplicata rodaria o
    gancho duas vezes. Compara o comando de verdade, nunca o JSON
    serializado: as aspas do comando novo viram \\" no texto e a busca por
    substring casa errado. True se removeu algo.
    """
    filtrada = [b for b in lista
                if not any(arquivo in c and c != comando_novo
                           for c in comandos_de(b))]
    mudou = len(filtrada) != len(lista)
    lista[:] = filtrada
    return mudou

MARCA_INICIO = "# === PÁGINAS (gerado — não edite à mão) ==="
MARCA_FIM = "# === FIM DAS PÁGINAS ==="
MARCA_MODULOS = "# === MÓDULOS (gerado — não edite à mão) ==="
MARCA_MODULOS_FIM = "# === FIM DOS MÓDULOS ==="


def paginas_no_disco(raiz: Path) -> dict:
    """Lê do disco os arquivos que devem estar embutidos.

    Página é texto; skill pode trazer asset binário (uma imagem, por exemplo).
    O que não decodifica como texto viaja como bytes — o padrão Agent Skills
    permite assets, e recusá-los deixaria a skill para trás.
    """
    achados = {}
    for padrao in FONTES:
        for caminho in sorted(raiz.glob(padrao)):
            if not caminho.is_file() or caminho.name == ".gitkeep":
                continue
            rotulo = caminho.relative_to(raiz).as_posix()
            try:
                achados[rotulo] = caminho.read_text(encoding="utf-8")
            except UnicodeDecodeError:
                achados[rotulo] = caminho.read_bytes()
    return achados


def modulos_no_disco(raiz: Path) -> dict:
    """Lê modulos/<nome>/ e devolve {nome: {caminho de destino: conteúdo}}.

    O caminho relativo dentro da pasta do módulo é o caminho de destino, tal e
    qual. Mesmo tratamento das páginas para o conteúdo: o que não decodifica
    como texto viaja como bytes, porque skill pode trazer asset binário.
    """
    achados = {}
    ...
    for pasta in sorted(p for p in origem.iterdir() if p.is_dir()):
        ...
        for caminho in sorted(pasta.rglob("*")):
            ...
            if rotulo == CARTAO_MODULO:
                continue
            try:
                arquivos[rotulo] = caminho.read_text(encoding="utf-8")
            except UnicodeDecodeError:
                arquivos[rotulo] = caminho.read_bytes()
        if arquivos:
            achados[pasta.name] = arquivos
    return achados


def trocar_bloco(linhas: list, inicio: str, fim: str, corpo: list) -> tuple:
    """Troca o miolo entre duas marcas. Devolve (linhas novas, mudou?).

    As posições são recalculadas a cada chamada de propósito: trocar um bloco
    move as linhas do outro, e índice guardado de antes apontaria para o lugar
    errado — do jeito mais silencioso possível, no meio de um dicionário.
    """
    try:
        i = next(n for n, l in enumerate(linhas) if l.startswith(inicio))
        f = next(n for n, l in enumerate(linhas) if l.startswith(fim))
    except StopIteration:
        sys.exit(f"Marca não encontrada neste arquivo: {inicio}")
    return linhas[:i] + corpo + linhas[f:], "".join(corpo) != "".join(linhas[i:f])


def bloco_de(marca: str, nome: str, itens: dict) -> list:
    """As linhas de um dicionário embutido, com a marca de abertura na frente."""
    return ([f"{marca}\n", f"{nome} = {{\n"]
...
def sincronizar(raiz: Path) -> int:
    """Reescreve as seções geradas deste arquivo a partir dos arquivos reais."""
    eu = Path(__file__)
    linhas = eu.read_text(encoding="utf-8").splitlines(keepends=True)

    achadas = paginas_no_disco(raiz)
    if not achadas:
        sys.exit("Nenhuma página encontrada. Rode na raiz do repositório da camada.")
    modulos = modulos_no_disco(raiz)

    linhas, mudou_paginas = trocar_bloco(
        linhas, MARCA_INICIO, MARCA_FIM, bloco_de(MARCA_INICIO, "PAGINAS", achadas))
    linhas, mudou_modulos = trocar_bloco(
        linhas, MARCA_MODULOS, MARCA_MODULOS_FIM,
        bloco_de(MARCA_MODULOS, "MODULOS", modulos))

    # O número de versão sobe sozinho, mas só quando o conteúdo mudou de
    # verdade — sincronizar sem mudança não fabrica versão nova.
    versao = VERSAO
    if mudou_paginas or mudou_modulos:
        maior, menor = VERSAO.split(".")
        versao = f"{maior}.{int(menor) + 1}"
        for n, l in enumerate(linhas):
            if l.startswith("VERSAO = "):
                linhas[n] = f'VERSAO = "{versao}"\n'
                break

    eu.write_text("".join(linhas), encoding="utf-8", newline="\n")
    for rotulo in achadas:
        print(f"  embutida: {rotulo}")
    print(f"\n{len(achadas)} páginas sincronizadas — camada {versao}.")

    if modulos:
        print(f"\nMódulos de {PASTA_MODULOS}/ — embutidos, mas nenhum viaja sozinho:")
        for nome, arquivos in modulos.items():
            print(f"  embutido: {nome} — {len(arquivos)} arquivos")

    # Editar skill também passa por aqui — sem o espelho junto, o Claude Code
    # seguiria lendo a versão velha em .claude/skills até alguém rodar o montar.
    print(f"\nSkills de {ORIGEM_SKILLS} espelhadas para {COPIA_SKILLS}")
...
def espelhar_skills(raiz: Path) -> list:
    """Copia as skills de .agents/skills para .claude/skills.

    A fonte é .agents/skills — o caminho que quase todo agente lê. O Claude
    Code lê só o seu próprio diretório, então recebe uma cópia. A cópia é
    derivada: sobrescrita quando difere da fonte, e o que sumiu da fonte sai
    dela também — senão o Claude seguiria lendo conteúdo já apagado.
    """
    ...
    for arquivo in sorted(origem.rglob("*")):
        if arquivo.is_dir() or arquivo.name == ".gitkeep":
            continue
        ...
        mexidos.append(("copiado", alvo.relative_to(raiz).as_posix()))
    # A limpeza vale só dentro de skill que a fonte ainda tem. Skill inteira
    # que só existe na cópia foi escrita à mão no lugar errado — o verificador
    # manda mover, e apagar aqui destruiria a única versão.
    if destino.is_dir():
        for arquivo in sorted(destino.rglob("*")):
            if arquivo.is_dir() or arquivo.name == ".gitkeep":
                continue
            rel = arquivo.relative_to(destino)
            if (origem / rel.parts[0]).is_dir() and not (origem / rel).exists():
                arquivo.unlink()
                mexidos.append(("removido", (destino / rel).relative_to(raiz).as_posix()))
    return mexidos


def espelhar_e_relatar(raiz: Path) -> None:
    mexidos = espelhar_skills(raiz)
    for acao, caminho in mexidos:
        print(f"  {acao}: {caminho}")
    if not mexidos:
        print("  nada a espelhar (as cópias já estavam em dia)")


def garantir_linhas(destino: Path, linhas, rotulo: str,
                    motivo: str = MOTIVO_IGNORAR) -> None:
    """Acrescenta as linhas que faltam. Não reescreve o que já está lá.

    O `escrever` mantém arquivo existente — o que aqui seria perigoso: um
    `.gitignore` que já existe deixaria `.credenciais/` de fora e o segredo
    entraria no git na primeira adição em massa.
    """
    texto = destino.read_text(encoding="utf-8") if destino.exists() else ""
    faltando = [l for l in linhas if l not in texto.splitlines()]
    if not faltando:
        print(f"  em dia:  {rotulo}")
        return
    ...
    if novo:
        novo += "\n"
    # O comentário entra uma vez só: rodar de novo com uma pasta nova não pode
    # deixar o arquivo cheio da mesma linha repetida.
    if motivo not in texto:
        novo += motivo + "\n"
    novo += "\n".join(faltando) + "\n"
    destino.write_text(novo, encoding="utf-8", newline="\n")
    print(f"  ajustado: {rotulo} (+{', '.join(faltando)})")


def garantir_gitignore_do_esqueleto(raiz: Path) -> None:
    """Migra as linhas antigas do esqueleto para a forma que preserva a pasta.

    `projetos/` escondia qualquer pasta homônima em qualquer profundidade;
    `/projetos/` escondia a pasta inteira — e pasta ignorada não chega no
    clone. A forma atual ignora só o conteúdo, e as exceções do LEIAME
    seguram a estrutura no git.
    """
    destino = raiz / ".gitignore"
    if not destino.exists():
        return
    trocas = {}
    for pasta in ("projetos", ".credenciais", "recursos"):
        trocas[f"{pasta}/"] = f"/{pasta}/*"
        trocas[f"/{pasta}/"] = f"/{pasta}/*"
    linhas = destino.read_text(encoding="utf-8").splitlines()
    trocadas = []
    for i, linha in enumerate(linhas):
        if linha in trocas:
            linhas[i] = trocas[linha]
            trocadas.append(linhas[i])
    if trocadas:
        destino.write_text("\n".join(linhas) + "\n", encoding="utf-8", newline="\n")
        print(f"  ajustado: .gitignore (esqueleto preserva a pasta: {', '.join(trocadas)})")
    # Quem tem o esqueleto precisa também das exceções — sem elas, os LEIAMEs
    # seguem ignorados e a estrutura continua não chegando no clone.
    if any(l in trocas.values() for l in linhas):
        garantir_linhas(destino, IGNORAR, ".gitignore (exceções do esqueleto)")


def garantir_devin(raiz: Path) -> None:
    """Desliga a ponte para a .claude e garante as negações próprias do Devin.

    Pela ponte, cada skill chegava em dobro (fonte + espelho) e permissão
    nenhuma vinha junto — o import não as traz. Config que já existe o
    `escrever` mantém; a emenda é feita aqui, preservando o resto.
    """
    destino = raiz / ".devin/config.json"
    if not destino.exists():
        return
    cfg = json.loads(destino.read_text(encoding="utf-8"))
    ponte = cfg.get("read_config_from", {}).get("claude")
    if ponte is False and "permissions" in cfg:
        print("  em dia:  configuração do Devin (ponte desligada)")
        return
    cfg.setdefault("read_config_from", {})["claude"] = False
    cfg.setdefault("permissions", json.loads("{" + PERMISSOES + "}")["permissions"])
    destino.write_text(json.dumps(cfg, indent=2, ensure_ascii=False) + "\n",
                       encoding="utf-8", newline="\n")
    print("  ajustado: .devin/config.json (ponte desligada, negações próprias)")


def garantir_espelho_no_git(raiz: Path) -> None:
    """Tira a linha skills/ do .claude/.gitignore de quem já tem a camada.

    A cópia era mantida fora do git; sessão na nuvem só enxerga o commitado,
    então a regra mudou — e arquivo existente o `escrever` não corrige.
    """
    destino = raiz / ".claude/.gitignore"
    if not destino.exists():
        return
    linhas = destino.read_text(encoding="utf-8").splitlines()
    if "skills/" not in linhas:
        return
    novo = "\n".join(l for l in linhas if l != "skills/")
    destino.write_text(novo + ("\n" if novo else ""), encoding="utf-8", newline="\n")
    print("  ajustado: .claude/.gitignore (espelho de skills agora entra no git)")


def garantir_gancho(raiz: Path) -> None:
    """Liga o gancho da qualidade no settings.json sem mexer no resto.

    O `escrever` mantém arquivo que existe — e o settings.json quase sempre
    existe. Sem esta emenda, repositório que já recebeu a camada nunca
    ganharia o gancho.
    """
    destino = raiz / ".claude/settings.json"
    cfg = json.loads(destino.read_text(encoding="utf-8")) if destino.exists() else {}
    inicio = cfg.setdefault("hooks", {}).setdefault("SessionStart", [])
    migrou = tirar_outras_formas(inicio, "injetar-qualidade.py", COMANDO_GANCHO)
    if tem_comando(inicio, COMANDO_GANCHO):
        if not migrou:
            print("  em dia:  gancho da qualidade no settings.json")
            return
    else:
        inicio.append({"hooks": [{"type": "command", "command": COMANDO_GANCHO}]})
    destino.parent.mkdir(parents=True, exist_ok=True)
    destino.write_text(json.dumps(cfg, indent=2, ensure_ascii=False) + "\n",
                       encoding="utf-8", newline="\n")
    print("  ligado:  gancho da qualidade no settings.json")


def garantir_lista_protegida(raiz: Path) -> None:
    """Cria a lista de branches protegidas em quem já tinha a camada.

    `--atualizar` não escreve ARQUIVOS, de propósito — é o que preserva o
    AGENTS.md de cada casa. Sem esta porta, o repositório antigo receberia o
    gancho e nunca veria o arquivo que ele lê.
    """
    destino = raiz / ".claude/branches-protegidas.txt"
    if destino.exists():
        print("  em dia:  lista de branches protegidas")
        return
    escrever(destino, LISTA_PROTEGIDA, ".claude/branches-protegidas.txt")
...
def garantir_ponteiro_das_regras(raiz: Path) -> None:
    """Acrescenta ao AGENTS.md o endereço da lista numerada, se ele não o tiver.

    É a segunda porta, e ela existe porque a primeira não passa. O
    `--atualizar` reescreve as páginas do guia e **nunca** o `AGENTS.md`, de
    propósito: a primeira regra de um repositório público é o oposto da de um
    privado. A consequência foi medida num repositório real — a lista de
    regras chegava atualizada ao disco a cada atualização, e nenhuma sessão a
    lia, porque nada mandava ler. Regra nova só valia para quem montasse do
    zero depois dela.

    Daí o formato. O que se acrescenta é o ENDEREÇO, nunca as regras: a lista
    tem uma casa só, e uma cópia dela envelheceria torta justamente do lado
    que a atualização não reescreve. Acrescenta uma vez e para; nunca
    reescreve, nunca reordena, nunca toca no que é do repositório.
    """
    destino = raiz / "AGENTS.md"
    if not destino.exists():
        print("  pulado: AGENTS.md não existe aqui")
        return
    texto = destino.read_text(encoding="utf-8")
    if "regras-da-camada" in texto:
        print("  em dia:  o AGENTS.md aponta a lista de regras")
        return
    if not texto.endswith("\n"):
        texto += "\n"
    destino.write_text(texto + PONTEIRO_DAS_REGRAS, encoding="utf-8", newline="\n")
    print("  acrescentado: o endereço da lista de regras, no fim do AGENTS.md")


def garantir_veto(raiz: Path) -> None:
    """Liga o veto de branch protegida, sem mexer no resto do settings.json.

    Regra de texto erra dos dois lados: `git push origin +x` é forçado e
    `git push origin x` não é, e nenhum padrão separa os dois sem errar de um
    lado. O gancho lê o comando e separa. Como todo gancho, ele falha aberto —
    se não subir, a regra 12 continua valendo só como texto.
    """
    # Só liga o que existe: declaração apontando para arquivo que não está lá
    # falha em silêncio — o gancho não roda e nada avisa.
    if not (raiz / ".claude/hooks/vetar-branch-protegida.py").exists():
        print("  pulado:  veto de branch protegida (o gancho não está no disco)")
        return

    destino = raiz / ".claude/settings.json"
    cfg = json.loads(destino.read_text(encoding="utf-8")) if destino.exists() else {}
    antes = cfg.setdefault("hooks", {}).setdefault("PreToolUse", [])
    migrou = tirar_outras_formas(antes, "vetar-branch-protegida.py", COMANDO_VETO)
    if tem_comando(antes, COMANDO_VETO):
        if not migrou:
            print("  em dia:  veto de branch protegida no settings.json")
            return
    else:
        antes.append({"matcher": "Bash|PowerShell",
                      "hooks": [{"type": "command", "command": COMANDO_VETO}]})
    destino.parent.mkdir(parents=True, exist_ok=True)
    destino.write_text(json.dumps(cfg, indent=2, ensure_ascii=False) + "\n",
                       encoding="utf-8", newline="\n")
    print("  ligado:  veto de branch protegida no settings.json")


def garantir_conferencia_mcp(raiz: Path) -> None:
    """Liga a conferência do .mcp.json na abertura, sem mexer no resto.

    Servidor MCP com caminho morto não sobe e não avisa — some da lista de
    ferramentas como se nunca tivesse existido. O gancho confere os caminhos
    declarados na abertura da sessão e fala quando um sumiu.
    """
    # Só liga o que existe: declaração apontando para arquivo que não está lá
    # é exatamente o defeito que este gancho existe para acusar.
    if not (raiz / ".claude/hooks/conferir-mcp.py").exists():
        print("  pulado:  conferência de MCP (o gancho não está no disco)")
        return

    destino = raiz / ".claude/settings.json"
    cfg = json.loads(destino.read_text(encoding="utf-8")) if destino.exists() else {}
    inicio = cfg.setdefault("hooks", {}).setdefault("SessionStart", [])
    migrou = tirar_outras_formas(inicio, "conferir-mcp.py", COMANDO_CONFERIR_MCP)
    if tem_comando(inicio, COMANDO_CONFERIR_MCP):
        if not migrou:
            print("  em dia:  conferência de MCP no settings.json")
            return
    else:
        inicio.append({"hooks": [{"type": "command", "command": COMANDO_CONFERIR_MCP}]})
    destino.parent.mkdir(parents=True, exist_ok=True)
    destino.write_text(json.dumps(cfg, indent=2, ensure_ascii=False) + "\n",
                       encoding="utf-8", newline="\n")
    print("  ligado:  conferência de MCP no settings.json")


def montar_esqueleto(raiz: Path) -> None:
    """Cria as pastas do workspace — mas só quando --esqueleto foi pedido."""
    if "--esqueleto" not in sys.argv:
        print("  fica de fora sem --esqueleto (é para a raiz de um workspace)")
        return
    for caminho, conteudo in ESQUELETO.items():
        escrever(raiz / caminho, conteudo, caminho)
    garantir_linhas(raiz / ".gitignore", IGNORAR, ".gitignore")


def modulos_pedidos() -> list:
    """Os nomes passados em `--modulo <nome>`. Repetível: dois módulos, duas vezes."""
    pedidos = []
    for n, arg in enumerate(sys.argv):
        if arg == "--modulo" and n + 1 < len(sys.argv):
            pedidos.append(sys.argv[n + 1])
        elif arg.startswith("--modulo="):
            pedidos.append(arg.split("=", 1)[1])
    return pedidos


def modulo_instalado(raiz: Path, nome: str) -> bool:
    """Instalado = algum arquivo daquele módulo existe aqui.

    Presença no disco, nunca arquivo de estado: estado é mais uma coisa para
    dessincronizar, e a pergunta que importa — "isto já está aqui?" — o disco
    responde sozinho e sem mentir.
    """
    return any((raiz / caminho).exists() for caminho in MODULOS.get(nome, {}))


def e_molde(caminho: str) -> bool:
    """O destino é território de quem usa: a camada escreve uma vez e nunca mais.

    A fronteira não é nova. Em conhecimento/, o primeiro nível é da camada e a
    subpasta é da casa — está no mapa do repositório. Um módulo pode entregar
    o molde vazio de uma memória que nasce ali; sobrescrever na atualização
    apagaria o que ninguém tem como recuperar.
    """
    partes = caminho.split("/")
    return len(partes) > 2 and partes[0] == "conhecimento"


def instalar_modulo(raiz: Path, nome: str, sobrescrever: bool) -> None:
    for caminho, conteudo in MODULOS[nome].items():
        if sobrescrever and not e_molde(caminho):
            atualizar_arquivo(raiz, caminho, conteudo)
        else:
            escrever(raiz / caminho, conteudo, caminho)


def conferir_modulos_pedidos() -> None:
    """Recusa nome de módulo que não existe — antes de escrever o primeiro byte.

    A conferência mora na entrada de propósito. Feita lá adiante, na hora de
    instalar, a montagem já teria criado dezenas de arquivos antes de dizer
    que o nome estava errado — e quem digitou errado leria a falha como se o
    resto tivesse falhado junto.
    """
    if desconhecidos := [n for n in modulos_pedidos() if n not in MODULOS]:
        sys.exit(f"Módulo que não existe nesta camada: {', '.join(desconhecidos)}.\n"
                 f"Disponíveis: {', '.join(sorted(MODULOS)) or 'nenhum'} "
                 "(veja com: python montar.py --modulos)")


def montar_modulos(raiz: Path, sobrescrever: bool) -> None:
    """Instala o que `--modulo` pediu; na atualização, também o que já está aqui.

    Sem bandeira e sem módulo instalado, nenhum byte é escrito. É o contrato da
    parte opcional: ela chega onde foi pedida, e em lugar nenhum mais.
    """
    pedidos = modulos_pedidos()
    # dict.fromkeys tira repetido e guarda a ordem: pedido primeiro, instalado
    # depois, e um módulo pedido que já está aqui não aparece duas vezes.
    ja_aqui = [n for n in MODULOS if modulo_instalado(raiz, n)] if sobrescrever else []
    alvos = list(dict.fromkeys(pedidos + ja_aqui))
    if not alvos:
        print("  nenhum — módulo entra por `--modulo <nome>` e por mais nada")
        return
    for nome in alvos:
        print(f"  módulo {nome}:")
...
def listar_modulos(raiz: Path) -> int:
    """`--modulos`: o que a camada oferece e o que este repositório já tem."""
    if not MODULOS:
        print(f"A camada {VERSAO} não traz nenhum módulo opcional.")
        return 0
    print(f"Módulos opcionais da camada {VERSAO}, em: {raiz}\n")
    for nome, arquivos in sorted(MODULOS.items()):
        ...
        elif presentes == len(arquivos):
            estado = f"instalado ({presentes} arquivos)"
        else:
            estado = f"instalado pela metade ({presentes} de {len(arquivos)})"
        print(f"  {nome:<20} {estado}")
    print("\nInstalar:   python montar.py --modulo <nome>")
    print("Atualizar:  python montar.py --atualizar  (só o que já está instalado)")
    return 0


def atualizar(raiz: Path) -> int:
    """Reescreve o que é derivado da camada. Não toca no que é do repositório.

    É o que permite um repositório irmão receber a camada de novo sem perder o
    que é dele. As instruções ficam de fora de propósito: a primeira regra de
    um repositório público é o oposto da de um privado, e sobrescrever isso
    copiaria a regra errada para o lado errado.
    """
    print(f"Atualizando a camada em: {raiz} — camada {VERSAO}\n")
    ...
    print(f"\n4. Skills de {ORIGEM_SKILLS} espelhadas para {COPIA_SKILLS}")
    espelhar_e_relatar(raiz)

    garantir_ajustes(raiz, 5)

    print("\nNão foram REESCRITOS, de propósito: AGENTS.md e CLAUDE.md — instrução")
    print("é de cada repositório. A única coisa que a atualização acrescenta ao")
    print("AGENTS.md é o endereço da lista de regras, e só se ele não o tiver:")
    print("sem isso, regra nova nunca chegaria a quem já montou a camada.")
    print("Na configuração, só os ajustes acima são garantidos.")
    print("Módulo que não está aqui não é instalado por --atualizar: peça pelo")
    print("nome (python montar.py --modulos mostra quais existem).")
    return 0


def garantir_ajustes(raiz: Path, numero: int) -> None:
    """Os ajustes que valem em qualquer repositório, e a conferência do final.

    Montar e atualizar terminam exatamente igual — só muda o número da seção.
    A sequência escrita nos dois lugares era o convite para alguém acrescentar
    um ajuste num lado e esquecer do outro.
    """
    print(f"\n{numero}. Ajustes garantidos na configuração")
    garantir_gancho(raiz)
    garantir_ponteiro_das_regras(raiz)
    garantir_lista_protegida(raiz)
    garantir_veto(raiz)
    garantir_conferencia_mcp(raiz)
    garantir_espelho_no_git(raiz)
    garantir_devin(raiz)
    garantir_gitignore_do_esqueleto(raiz)
    garantir_linhas(raiz / ".gitignore", IGNORAR_SITE, ".gitignore (site)",
                    MOTIVO_SITE)
    garantir_linhas(raiz / ".gitignore", IGNORAR_LIXO, ".gitignore (descartáveis)",
                    MOTIVO_LIXO)

    print(f"\n{numero + 1}. O git enxerga o que a camada escreveu?")
    conferir_versionaveis(raiz)


def caminhos_da_camada(raiz: Path) -> list:
    """Tudo o que este script escreve e que precisa entrar no git.

    Só o que existe no disco: o esqueleto não nasce sem --esqueleto, e cobrar
    arquivo que ninguém pediu viraria aviso que se aprende a ignorar.
    """
    caminhos = list(ARQUIVOS) + list(ESQUELETO) + list(PAGINAS)
    for arquivos in MODULOS.values():
        caminhos += list(arquivos)
    copia = raiz / COPIA_SKILLS
    if copia.exists():
        caminhos += [p.relative_to(raiz).as_posix()
                     for p in copia.rglob("*") if p.is_file()]
    return sorted({c for c in caminhos if (raiz / c).exists()})


def conferir_versionaveis(raiz: Path) -> None:
    """Acusa arquivo da camada que o .gitignore do repositório esconde.

    Escrever o arquivo não basta. Uma linha de ignore sem barra inicial
    esconde em qualquer profundidade, e aí o que a camada criou não chega ao
    clone nem à sessão na nuvem — que só enxerga o que foi commitado. A falha
    é silenciosa: o repositório parece montado e não está.

    Avisa, não conserta. Dois motivos: as regras de ignore são do repositório,
    não da camada; e o git não reinclui arquivo cujo diretório-pai está
    excluído, então uma exceção automática mentiria em parte dos casos.
    """
    caminhos = caminhos_da_camada(raiz)

    # -z para entrar e sair: campos separados por NUL, caminho sem aspas. Em
    # texto, o Python troca \n por \r\n ao escrever no stdin do filho no
    # Windows, e o git passa a receber caminho terminado em \r — que casa com
    # a regra errada. Bytes e NUL tiram os dois problemas de uma vez.
    entrada = "\0".join(caminhos).encode("utf-8")
    try:
        r = subprocess.run(["git", "check-ignore", "-z", "-v", "--stdin"],
                           cwd=raiz, input=entrada, capture_output=True)
    except OSError:
        print("  pulado: git não encontrado — o ignore não pôde ser conferido")
        return
    # 0 = achou ignorado; 1 = nenhum ignorado. Qualquer outro é erro do git.
    if r.returncode not in (0, 1):
        print(f"  pulado: git não respondeu ({r.stderr.decode(errors='replace')[:60]})")
        return

    campos = r.stdout.decode("utf-8", "replace").split("\0")
    escondidos = []
    for i in range(0, len(campos) - 3, 4):
        fonte, numero, padrao, caminho = campos[i:i + 4]
        # Com -v o git também relata o caminho cuja regra vencedora é uma
        # NEGAÇÃO (`!/tmp/LEIAME.md`) — que diz o contrário de ignorado. Ler
        # essa linha como "escondido" acusa arquivo que está são.
        if padrao.startswith("!"):
            continue
        escondidos.append((caminho, f"{fonte}:{numero}:{padrao}"))

    if not escondidos:
        print(f"  em dia:  o git versiona os {len(caminhos)} arquivos da camada")
        return

    print(f"  AVISO:   {len(escondidos)} de {len(caminhos)} arquivos da camada")
    print("           existem no disco e o git os ignora — não chegam ao clone:")
    for caminho, regra in escondidos:
        print(f"             {caminho}  <-  {regra}")
    print("           Conserte a regra (a barra inicial ancora na raiz) ou abra")
    print("           a exceção. Este script não mexe no ignore que é seu.")
...
def gravar(destino: Path, conteudo) -> None:
    ...
    if isinstance(conteudo, bytes):
        ...
        return
    # newline="\n" impede o Windows de trocar as quebras de linha por \r\n:
    # sem isso o mesmo script gera arquivos diferentes em cada sistema.
    destino.write_text(conteudo, encoding="utf-8", newline="\n")


def escrever(destino: Path, conteudo, rotulo: str) -> None:
    if destino.exists():
        print(f"  mantido: {rotulo}")
        return
...
def atualizar_arquivo(raiz: Path, caminho: str, conteudo) -> None:
    """Traz a versão da camada por cima, dizendo o que fez.

    É o par do `escrever`: aquele mantém o que já existe, este troca. Quem
    escolhe entre os dois é quem chama — a montagem preserva, a atualização
    substitui.
    """
    ...
    if atual == conteudo:
        print(f"  em dia:  {caminho}")
        return
    gravar(destino, conteudo)
    print(f"  {'trocado' if atual is not None else 'criado '}: {caminho}")


def main() -> int:
    if "--versao" in sys.argv:
        print(f"camada {VERSAO}")
        return 0

    raiz = Path.cwd()
    if not (raiz / ".git").exists():
        sys.exit("Rode na raiz do repositório: pasta .git não encontrada aqui.")

    if "--modulos" in sys.argv:
        return listar_modulos(raiz)

    conferir_modulos_pedidos()

    if "--sincronizar" in sys.argv:
        print(f"Sincronizando as páginas de: {raiz}\n")
        return sincronizar(raiz)

    if "--atualizar" in sys.argv:
        return atualizar(raiz)

    print(f"Montando em: {raiz} — camada {VERSAO}\n")

    print("1. Configuração para agentes de IA")
    ...
    print(f"\n5. Skills de {ORIGEM_SKILLS} espelhadas para {COPIA_SKILLS}")
    espelhar_e_relatar(raiz)

    garantir_ajustes(raiz, 6)

    print("\nPronto.")
    # Um comando por linha: `&&` é erro de sintaxe no Windows PowerShell 5.1,
    # e instrução que não roda no terminal de quem lê não é instrução.
    print("Para ver o guia como site, um comando por linha:")
    print("  cd site")
    print("  npm install")
    print("  npm run build")
    return 0
