...
PROBLEMA_CUSTO = "argumento --custo: não é JSON ({})"
...
AJUDA_TURNOS = ("turnos que a sessão gastou (num_turns); opcional, "
...
def _tipo_certo(tipo: str, dado) -> bool:
    if isinstance(dado, bool):
        return tipo == "boolean"
    if tipo == "integer" and isinstance(dado, float):
...
def _custo_do_argumento(texto):
    if not texto:
        return None, ""
    try:
        return json.loads(texto), ""
    except (ValueError, RecursionError) as quebra:
        return None, PROBLEMA_CUSTO.format(quebra)
...
def _problemas_dos_argumentos(args, esquema: dict) -> list:
    ...
    custo, problema = _custo_do_argumento(getattr(args, "custo", None))
    if problema:
...
RECUSA = [
    ...
    ("suposto com item não-texto", _base(suposto=[{"x": 1}]),
     "deveria ser string"),
    ("custo com usd negativo", _base(custo={
        "usd": -1, "tokens": {"entrada": 0, "saida": 0,
...
def _o_codigo_manda_nos_campos(pasta, caso):
    ...
    _cli(["materializar", "--dir", pasta, "--trabalho", "t-turnos",
          "--etapa", "alfa", "--ordem", "1", "--teto", "3",
          "--turnos", "11"],
         entrada=json.dumps({"structured_output":
                             json.loads(EXEMPLO_DA_ISSUE)}))
    escrito = _ler(pasta, "t-turnos", "01-alfa-c1.json")
    caso("turnos vem do argumento — o carimbo é do código",
         escrito.get("turnos") == 11)
    _cli(["materializar", "--dir", pasta, "--trabalho", "t-sem-turnos",
          "--etapa", "alfa", "--ordem", "1", "--teto", "3"],
         entrada=json.dumps({"structured_output":
                             json.loads(EXEMPLO_DA_ISSUE)}))
    caso("sem o argumento o campo não nasce — opcional de verdade",
         "turnos" not in _ler(pasta, "t-sem-turnos", "01-alfa-c1.json"))
    com_forja = dict(json.loads(EXEMPLO_DA_ISSUE), turnos=999)
    _cli(["materializar", "--dir", pasta, "--trabalho", "t-turno-forjado",
          "--etapa", "alfa", "--ordem", "1", "--teto", "3"],
         entrada=json.dumps({"structured_output": com_forja}))
    caso("modelo não escreve turnos: o forjado some sem o argumento",
         "turnos" not in _ler(pasta, "t-turno-forjado", "01-alfa-c1.json"))

    custo = ('{"usd": 0.1234, "tokens": {"entrada": 10, "saida": 2, '
             '"cache-lido": 100, "cache-criado": 50}}')
    _cli(["materializar", "--dir", pasta, "--trabalho", "t-custo",
          "--etapa", "alfa", "--ordem", "1", "--teto", "3",
          "--custo", custo],
         entrada=json.dumps({"structured_output":
                             json.loads(EXEMPLO_DA_ISSUE)}))
    caso("custo vem do argumento — o carimbo é do código",
         _ler(pasta, "t-custo", "01-alfa-c1.json").get("custo")
         == json.loads(custo))
    com_forja = dict(json.loads(EXEMPLO_DA_ISSUE),
                     custo={"usd": 9.9, "tokens": {}})
    _cli(["materializar", "--dir", pasta, "--trabalho", "t-custo-forjado",
          "--etapa", "alfa", "--ordem", "1", "--teto", "3"],
         entrada=json.dumps({"structured_output": com_forja}))
    caso("modelo não escreve custo: o forjado some sem o argumento",
         "custo" not in _ler(pasta, "t-custo-forjado", "01-alfa-c1.json"))
    resposta = _cli(["materializar", "--dir", pasta,
                     "--trabalho", "t-custo-quebrado",
                     "--etapa", "alfa", "--ordem", "1", "--teto", "3",
                     "--custo", "{quebrado"],
                    entrada=json.dumps({"structured_output":
                                        json.loads(EXEMPLO_DA_ISSUE)}))
