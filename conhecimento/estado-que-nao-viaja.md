...
- O que a sessão precisa e não está no git se declara por nome e se verifica
  por instrumento — é o `verificar-ambiente.py`.
...
- `receita` — a página do repositório que ensina a repor; é o endereço que o
  aviso do gancho aponta.
...
`python .claude/hooks/verificar-ambiente.py --testar`.
