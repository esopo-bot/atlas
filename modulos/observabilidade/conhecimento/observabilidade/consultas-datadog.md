...
| Consulta | Responde o quê | Aplicação | Por que não é a óbvia | Quando mente |
| --- | --- | --- | --- | --- |
| `env:producao service:chapelaria-api status:error` | os erros da aplicação de pedidos | chapelaria | o serviço se chama `chapelaria-api`, não `chapelaria` — o nome da aplicação não existe na ferramenta | sem `env`, mistura homologação e parece o dobro do volume |
| `env:producao service:almoxarifado-api @http.status_code:[400 TO 499]` | recusa do cliente, separada de falha nossa | almoxarifado | `status:error` não pega 4xx: a aplicação registra 4xx como `info` | se a aplicação passar a registrar 4xx como `warn`, continua certa — é o código que filtra, não o nível |
